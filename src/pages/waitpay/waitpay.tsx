...
const waitpay = () => {
  ...
  const cancel=()=>{
    ...
    request({
      url: `/orderitems/updateorderstate?orderid=${orderid}&state=0`,
      method: 'POST',
      header: {
    ...
    request({
      ...
      if (res.message === "success") {
        const proItems = {
          ...
          createTime: formatTime(res.data.finishTime),
  ...
  const gotopay=()=>{
    ...
    request({
      url: `/orderitems/updateorderstatus?orderid=${orderid}&orderstate=2&paystate=2&paymethod=2`,
      method: 'POST',
      header: {
        //'token': `Bearer ${Taro.getStorageSync('token')}`,
        'Content-Type': 'application/json',
      ...
      if (res.message === "success"&& res.data === 1) {
        ...
        Taro.redirectTo({
          url: `/pages/paysuccess/paysuccess?orderid=${orderid}&price=${orderinfo.price}&paymethod=2`,
        });
      }else {
        Taro.showToast({
    ...
    <View className="confirm">
      {seconds > 0 &&
        <View className="payinfo-col">
          ...
          <View className="payinfo-between">
            <Text className="warning-text">需付 : ￥{parseFloat(orderinfo.price).toFixed(2)}</Text>
      ...
      <View>
        <View className="pro-row">
          <Image src={orderinfo.pimage} className="pro-image"></Image>
          <View className="pro-clo-info">
            <Text className="pro-title">{orderinfo.pname}</Text>
            <View className="pro-spec">
              <Text className="specvalue">
                {orderinfo.specvalueName}
              </Text>
            </View>
            <View className="pro-between">
              <Text className="pro-price">￥{(parseFloat(orderinfo.price) / orderinfo.quantity).toFixed(2)}</Text>
              <Text className="pro-quantity"> x {orderinfo.quantity}</Text>
            </View>
          </View>
        </View>
      </View>
      <View className="orderinfo-col">
        <View className="orderinfo-view">
          ...
          <Text className="orderinfo-numtext">{orderinfo.orderNum}</Text>
        </View>
        <View className="orderinfo-view">
          <Text className="orderinfo-text">下单时间：</Text>
          <Text className="orderinfo-numtext">{orderinfo.createTime}</Text>
        </View>
        <View className="orderinfo-view">
        ...
        <View className="orderinfo-view">
          ...
          <Text className="orderinfo-numtext">{orderinfo.remark}</Text>
        </View>
        <View className= "totalprice-view">
          <Text className= "totalprice">商品总额</Text>
          <Text className= "totalpricenum"> ￥ {parseFloat(orderinfo.price).toFixed(2)}</Text>
        </View>
        <View className= "shouldpay">
          <Text className= "shouldpayname">应付金额 {'\u00A0'}{'\u00A0'}<Text className= "shouldpaynum">￥ {parseFloat(orderinfo.price).toFixed(2)}</Text></Text>
        </View>
      </View>
      {seconds > 0 &&
        <View className="actions2">
          ...
          <View className="actions2-viewout2">
            <AtButton className="action2-button-buy" onClick={gotopay}>支付 ￥ {parseFloat(orderinfo.price).toFixed(2)}</AtButton>
          </View>
        </View>
      }
      {seconds <= 0 &&
